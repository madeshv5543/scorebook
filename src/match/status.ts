...
export default (match: Match) => {
    const battingTeamRunsToWin = () => {
        ...
        const { battingTeamRuns, bowlingTeamRuns } = match.innings.reduce(
            (scoreObj, i) => ({
        ...
        if (battingTeamRuns > bowlingTeamRuns) {
            return `${getTeam(match, innings.battingTeam).name} won by ${battingTeamRuns - bowlingTeamRuns}`;
        }
        return `${getTeam(match, innings.battingTeam).name} need ${bowlingTeamRuns - battingTeamRuns + 1} to win`;
    };

    const leadingTeam = () => {
        const { homeTeamRuns, awayTeamRuns } = match.innings.reduce(
            (scoreObj, i) => ({
        ...
        return `${leaders.name} won by ${leaders.by}`;
    };
